...
export function CommunityCodeCard() {
  ...
  const shareLink = isClient ? `${window.location.origin}/signup?ref=${shareCode}` : '';
  const shareMessage = `Hey! I'm using KitaMo to manage my finances and it's been great. You should check it out! You can use my share code when you get started: ${shareCode}. Link: ${shareLink}`;

  const shareActions = [
    { name: 'WhatsApp', icon: MessageCircle, url: `https://wa.me/?text=${encodeURIComponent(shareMessage)}`},
    { name: 'Telegram', icon: Send, url: `https://t.me/share/url?url=${encodeURIComponent(shareLink)}&text=${encodeURIComponent(shareMessage)}` },
    { name: 'Messenger', icon: Share2, url: `https://www.facebook.com/dialog/send?app_id=YOUR_FACEBOOK_APP_ID&link=${encodeURIComponent(shareLink)}&redirect_uri=${encodeURIComponent(isClient ? window.location.href : '')}` }
  ];
